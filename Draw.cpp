...
void CDraw::Box(float x, float y, float w, float h, float linewidth, DWORD color)
{
	...
	V[0].z = V[1].z = V[2].z = V[3].z = V[4].z = 0;
	V[0].rhw = V[1].rhw = V[2].rhw = V[3].rhw = V[4].rhw = linewidth;
	...
	pDevice->CreateVertexBuffer((4 + 2) * sizeof(vertex), D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &g_pVB, NULL);

	VOID* pVertices;
	g_pVB->Lock(0, (4 + 2) * sizeof(vertex), (void**)&pVertices, 0);
	memcpy(pVertices, &V, (4 + 2) * sizeof(vertex));
	g_pVB->Unlock();
